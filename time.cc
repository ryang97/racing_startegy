...
Time::Time(int startLap, int endLap, char tyre, float fuelComsumption, float softTime,
...
float Time::stintTime() {
  ...
  for ( int i = 0; i < lap ; ++i ) {
    if ((tyre == 'S') or (tyre == 's'))
      totalTime = totalTime + softTime + softLapTime(i)
                  - fuelConsumption * (startLap - 1 ) * 0.03; // every 10 kg fuel saves 0.3 
                                                              // second per lap
    else if ((tyre == 'M') or (tyre == 'm'))
      totalTime = totalTime + medTime + medLapTime(i)
                  - fuelConsumption * (startLap - 1 ) * 0.03;  
